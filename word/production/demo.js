...
//load images  to make  
var el = document.getElementById('myloadImagesSection');

function myloadImages(isReplay) {
  document.getElementById('mymyresult').value="";
  var imgs = ['1'];
        ...
        if (req.readyState == 4) 
        {
          if (req.status == 200)
          {
			  ...
			  var mycars=new Array(14)
					...
					mycars["mil"]="军事";
					mycars["auto"]="汽车";
			  document.getElementById('mymyimg').style.display="none";
			  document.getElementById('mymyresult').style.display="block";
			  document.getElementById('mymyresult').value=req.responseText;
			  //var myttstr="class:"+mycars[document.getElementById('mymyresult').value];
			  //document.getElementById('mymyresult').value=myttstr;
            }, 500);
          }
        }
      };
       progressJs("img[data-img='" + imgName + "']").setOptions({overlayMode: true, theme: 'blueOverlayRadiusWithPercentBar'}).start().autoIncrease(1, 150);
      var myrul = document.getElementById('wc-panel-feed').url.value;
      req.open("GET",  "2.php?q="+myrul, true);
      req.send();
